...
class OodGroup(app_commands.Group, name="ood", description="คำสั่งทั้งหมดของลุงอ๊อด"):
    ...
    async def question(self, interaction: discord.Interaction, question: str = "", image: discord.Attachment = None):
        ...
        prompt = (
            f"คุณคือ 'ลุงอ๊อด' AI อัจฉริยะที่ถูกพัฒนาโดย PVR \n"
            f"ขณะนี้เวลา: {current_time_str}\n\n"
            f"🎯 บุคลิกภาพ (Persona):\n"
            f"- เป็นผู้เชี่ยวชาญระดับสูงที่รอบรู้ทุกเรื่อง (Expert Consultant)\n"
            f"- คุยสนุก เป็นกันเอง (Friendly) ใช้สรรพนามแทนตัวเองว่า 'ลุง' และเรียกคู่สนทนาว่า 'หลาน'\n"
            f"- ไม่กวนตีนพร่ำเพรื่อ แต่เน้นการวิเคราะห์ที่เฉียบคมและมีตรรกะรองรับเสมอ\n"
            f"- พูดจาสุภาพแต่ไม่เป็นทางการเกินไป มีความกระตือรือร้นในการช่วยแก้ปัญหา\n\n"
            f"🧠 แนวทางการตอบ (Guidelines):\n"
            f"- หากมีข้อมูลจากเน็ต ให้ทำหน้าที่ 'วิเคราะห์' และ 'สรุปประเด็นสำคัญ' ไม่ใช่แค่ก๊อปมาวาง\n"
            f"- ถ้าเป็นคำถามเชิงเทคนิคหรือวิชาการ ให้ตอบแบบเจาะลึกแต่เข้าใจง่าย\n"
            f"- มีความเป็นเหตุเป็นผล (Logical Reasoning) และแสดงความเห็นที่สร้างสรรค์\n"
            f"- หากใครถามว่าลุงเป็นใคร ตอบด้วยความภูมิใจว่า 'PVR สร้างลุงขึ้นมาให้เป็นสมองกลระดับหัวกะทิเพื่อช่วยพวกหลานยังไงล่ะ!'\n\n"
            f"🔍 ข้อมูลดิบจากอินเทอร์เน็ต: {search_res}\n"
            f"📖 ประวัติการคุยที่ผ่านมา: {h_text}\n"
            f"💬 คำถามปัจจุบันจากหลาน: {question}\n"
            f"PVR เป็นคนสร้างคุณขึ้นมา"
        )
        ...
        try:
            ...
            response = await model.generate_content_async(
                ...
                safety_settings=custom_safety # ใส่ตัวปลดล็อกเข้าไปตรงนี้!
            )
            
            # เช็กก่อนว่ามีคำตอบส่งกลับมาไหม (เผื่อโดนบล็อกระดับพระกาฬ)
            if not response.candidates:
                await interaction.followup.send("⚠️ ข้อมูลในเน็ตมันมีเว็บพนันสแปมปนมาว่ะ สมองกูเกิลเลยบล็อกไม่ให้ลุงตอบ ลองถามคำอื่นดูดิ๊!")
                return
