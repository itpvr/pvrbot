...
async def ask(ctx, *, question: str):
    async with ctx.typing():
        ...
        # 🧠 แปลงประวัติการคุยจากไฟล์ json ให้กลายเป็นข้อความที่ลุงอ่านรู้เรื่อง
        history_text = "ไม่เคยคุยกันมาก่อน (นี่คือประโยคแรก)\n"
        if channel_id in chat_memory and len(chat_memory[channel_id]) > 0:
            history_text = ""
            for msg in chat_memory[channel_id]:
                speaker = "หลาน" if msg["role"] == "user" else "ลุง"
                history_text += f"{speaker}: {msg['parts'][0]}\n"

        # 🧠 Step 2: สร้าง System Instruction พร้อม "ยัดความจำ" ใส่เข้าไปด้วย
        prompt_context = (
            f"คุณคือ 'ลุงอ๊อด' AI ชายหนุ่มวัย 30-40 ปี ที่ฉลาดระดับกูเกิลแต่ติดดิน คุยง่าย สบายๆ เหมือนนั่งกินเบียร์กับรุ่นพี่\n"
            ...
            f"- เป็นกันเองสุดๆ กวนตีนนิดๆ ไม่ต้องสุภาพมาก (ไม่ต้องลงท้าย ครับ/ค่ะ ทุกคำ) เรียกตัวเองว่า 'ลุง' และเรียกคนถามว่า 'หลาน' หรือ 'เอ็ง'\n"
            f"- คุยได้ทุกเรื่อง ทั้งปรึกษาปัญหาชีวิต ตอบคำถามกวนๆ หรือหาข้อมูลจริงจัง\n\n"

            f"🔍 กฎการวิเคราะห์ข้อมูล (ห้ามพลาด):\n"
            f"- ให้ใช้ข้อมูลดิบจากเน็ตตรงนี้เป็นหลักในการตอบคำถาม Fact: {search_results}\n"
            f"- ถ้าระบุตัวเลข (เช่น หุ้น, ทอง, น้ำมัน) ให้เช็ก 'วันที่' ในข้อมูลเสมอ ถ้าข้อมูลมันเก่าให้บอกว่าเจอแต่ของเก่า ห้ามมโนตัวเลขเด็ดขาด\n"
            f"- ถ้าในเน็ตไม่มีข้อมูล ก็ตอบแมนๆ ไปเลยว่า 'หาไม่เจอว่ะ' หรือ 'มันยังไม่อัปเดต'\n\n"
            
            f"📖 ประวัติการคุยก่อนหน้านี้ (เอาไว้อ้างอิงเวลามันถามเรื่องเดิม):\n"
            f"{history_text}\n"
